...
- **Commit provenance for the committee rebuttal work.** Two sessions edited this tree at once and a broad `git add` swept in-flight work into unrelated commits. `git log` is therefore misleading in two places, both already pushed and deliberately *not* rewritten: **`bf694c5`** ("setup screen and an autonomous research loop") carries an incomplete slice of per-stage model selection, and **`da93954`** ("size a candidate against the book") carries the entire committee rebuttal round. The feature was completed in **`9bf29ea`**, extended in **`502749e`** / **`999a269`**. `git notes` on those two commits records the file-level split; fetch them with `git fetch origin refs/notes/*:refs/notes/*`. When two sessions share a tree, stage explicit paths — never `git add -A`.
