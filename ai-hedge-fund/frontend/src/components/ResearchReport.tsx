...
export function ResearchReport() {
  ...
  const activeTab = searchParams.get("tab") ?? "Fundamental";
  ...
  const research = useQuery({
    ...
    queryFn: () =>
      api.checkTicker(ticker, {
        // Stage 02 is the measured evidence. The judged evidence — investor
        // views, the synthesis, signal intelligence — moved to stage 03, so
        // this screen no longer pays for a model run it does not display.
        includeAi: false,
      }),
    enabled: !!ticker,
  });
  ...
  // The analyst and sentiment feeds moved with signal intelligence to stage
  // 03, where there is a model view to read them against. Fetching them here
  // as well would be two requests whose result nothing on this screen shows.
  ...
  if (!ticker) {
    ...
    const resume =
      activeTicker && activeTicker !== ticker ? activeTicker : null;
    const others: string[] = recents
      .filter((r: string) => r !== resume)
      .slice(0, 5);
    return (
        ...
        <div className="flex grow items-center justify-center px-lg">
          <div className="w-full max-w-lg">
            ...
            <ChatInput
              ...
              placeholder="Enter a ticker symbol (e.g. AAPL, THYAO.IS)"
            />
            ...
            {resume && (
              ...
              >
                ...
                <span className="mt-2xs block font-display text-[18px] text-cadmium">
                  {resume}
                </span>
              </button>
            )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400">
                {(fundamentals.name as string) ?? ticker}
              </p>
            </div>
            {d?.price?.current != null && (
              <div className="text-right">
                <p className="text-3xl font-bold text-white font-mono">
                  $
                  {typeof d.price.current === "number"
                    ? d.price.current.toFixed(2)
                    : String(d.price.current)}
                </p>
                <p
                  className={`text-sm font-mono ${(d.price.change_30d_pct ?? 0) >= 0 ? "text-accent-green" : "text-accent-red"}`}
                >
                  {(d.price.change_30d_pct ?? 0) >= 0 ? "+" : ""}
                  {typeof d.price.change_30d_pct === "number"
                    ? d.price.change_30d_pct.toFixed(2)
                    : d.price.change_30d_pct}
                  % (30d)
                </p>
              </div>
            )}
          ...
          <ErrorBoundary>
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    items={[
                      [
                        ...
                        fundamentals.pe_ratio != null
                          ? (fundamentals.pe_ratio as number).toFixed(2)
                          : "N/A",
                      ],
                      [
                        "Forward P/E",
                        fundamentals.forward_pe != null
                          ? (fundamentals.forward_pe as number).toFixed(2)
                          : "N/A",
                      ],
                      [
                        "PEG Ratio",
                        fundamentals.peg_ratio != null
                          ? (fundamentals.peg_ratio as number).toFixed(2)
                          : "N/A",
                      ],
                      [
                        "P/B Ratio",
                        fundamentals.price_to_book != null
                          ? (fundamentals.price_to_book as number).toFixed(2)
                          : "N/A",
                      ],
                      [
                        "Div Yield",
                      ...
                      [
                        ...
                        fundamentals.beta != null
                          ? (fundamentals.beta as number).toFixed(2)
                          : "N/A",
                      ],
                      [
                        "Market Cap",
                        fundamentals.market_cap != null
                          ? `$${((fundamentals.market_cap as number) / 1e9).toFixed(1)}B`
                          : "N/A",
                      ],
                    ]}
                  />
                  <DataCard
                    ...
                    items={[
                      ...
                      [
                        "Net Income",
                        fmtLarge(fundamentals.net_income as number),
                      ],
                      ["EBITDA", fmtLarge(fundamentals.ebitda as number)],
                      ["FCF", fmtLarge(fundamentals.free_cash_flow as number)],
                      [
                        "Total Assets",
                        fmtLarge(fundamentals.total_assets as number),
                      ],
                      [
                        "Total Debt",
                        fmtLarge(fundamentals.total_debt as number),
                      ],
                    ]}
                  />
                  <DataCard
                    ...
                    items={[
                      [
                        "Current",
                        d?.price?.current != null
                          ? `$${(d.price.current as number).toFixed(2)}`
                          : "N/A",
                      ],
                      [
                        "52w High",
                        fundamentals["52w_high"] != null
                          ? `$${(fundamentals["52w_high"] as number).toFixed(2)}`
                          : "N/A",
                      ],
                      [
                        "52w Low",
                        fundamentals["52w_low"] != null
                          ? `$${(fundamentals["52w_low"] as number).toFixed(2)}`
                          : "N/A",
                      ],
                      [
                        "30d Change",
                        d?.price?.change_30d_pct != null
                          ? `${(d.price.change_30d_pct as number).toFixed(2)}%`
                          : "N/A",
                      ],
                    ]}
                  />
                ...
                <div className="grid grid-cols-3 gap-4">
                  <DataCard
                    ...
                    items={[
                      ...
                      [
                        "MACD Trend",
                        (technicals.macd as Record<string, unknown>)?.trend ??
                          "N/A",
                      ],
                      [
                        "MACD Line",
                        (technicals.macd as Record<string, unknown>)?.line,
                      ],
                      [
                        "MACD Signal",
                        (technicals.macd as Record<string, unknown>)?.signal,
                      ],
                      [
                        "MACD Hist",
                        (technicals.macd as Record<string, unknown>)?.histogram,
                      ],
                    ]}
                  />
                  <DataCard
                    ...
                    items={[
                      ...
                      [
                        "Above SMA50",
                        technicals.above_sma50 != null
                          ? technicals.above_sma50
                            ? "Yes"
                            : "No"
                          : "N/A",
                      ],
                      [
                        "Above SMA200",
                        technicals.above_sma200 != null
                          ? technicals.above_sma200
                            ? "Yes"
                            : "No"
                          : "N/A",
                      ],
                  ...
                  <DataCard
                    ...
                    items={[
                      ...
                      [
                        "BB Upper",
                        (technicals.bollinger as Record<string, unknown>)
                          ?.upper,
                      ],
                      [
                        "BB Middle",
            ...
            {activeTab === "Alt data" && (
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <InsiderFlowCard
                    data={insiderAlt.data?.transactions ?? []}
                    loading={insiderAlt.isLoading}
                  />
                  <CongressionalCard
                    data={congressional.data?.trades ?? []}
                    loading={congressional.isLoading}
                  />
                ...
                {!esg.data &&
                  ...
                  !congressional.data?.trades?.length &&
                  !insiderAlt.data?.transactions?.length &&
                  !insiderAlt.isLoading &&
                  !filings.isLoading && (
            ...
            {activeTab === "Peers" && (
              <div className="flex flex-col gap-4">
                ...
                {peers.data && (
                  <PeersTab data={peers.data} currentTicker={ticker} />
                )}
                    ...
                    <div className="bg-surface-card rounded-xl p-4">
                      ...
                      {newsData.data.articles.map(
                        (n: Record<string, unknown>, i: number) => (
                          <div
                            key={i}
                            className="py-2 border-t border-border first:border-0"
                          >
            ...
            {activeTab === "Institutional" && (
              <div className="flex flex-col gap-4">
                {institutional.data && (
                  <InstitutionalCard data={institutional.data.holders} />
                )}
                {insider.data && (
                  <InsiderCard data={insider.data.transactions} />
                )}
          ...
          <aside className="w-72 shrink-0 border-l border-border p-4 overflow-y-auto flex flex-col gap-4">
            ...
            <div className="bg-surface-card rounded-xl p-4">
              ...
              <p className="text-sm text-white">
                {(fundamentals.sector as string) ?? "N/A"}
              </p>
              <p className="text-xs text-gray-500 mt-1">
                {(fundamentals.industry as string) ?? ""}
              </p>
            </div>
            <div className="bg-surface-card rounded-xl p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">
              ...
              <p className="text-sm text-white">
                {(fundamentals.exchange as string) ?? "N/A"}
              </p>
              <p className="text-xs text-gray-500 mt-1">
                {(fundamentals.currency as string) ?? "USD"}
              </p>
            </div>
            {d?.news_sentiment && (
              <div className="bg-surface-card rounded-xl p-4">
                <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">
                  Headline sentiment (FinBERT)
                </p>
                {d.news_sentiment.enabled ? (
                  <>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      mean signed · {d.news_sentiment.aggregate.article_count}{" "}
                      headlines
                    </p>
                    <p
                      className="text-label text-gray-600 mt-2 truncate"
                      title={d.news_sentiment.model ?? ""}
                    >
    ...
    <div className="bg-surface-card rounded-xl p-4">
      ...
      <div className="grid grid-cols-4 gap-4 mb-4">
        ...
        <div>
          ...
          <p className="text-lg font-bold text-white font-mono">
            {data.target_mean ? `$${data.target_mean}` : "N/A"}
          </p>
        </div>
        <div>
          <p className="text-xs text-gray-500">Upside</p>
          <p
            className={`text-lg font-bold font-mono ${(data.upside_pct ?? 0) >= 0 ? "text-accent-green" : "text-accent-red"}`}
          >
            ...
            <tbody>
              {data.earnings_dates.slice(0, 6).map((e, i) => (
                <tr key={i} className="border-b border-border/50">
                  ...
                  <td
                    className={`text-right font-mono ${(e.surprise_pct ?? 0) >= 0 ? "text-accent-green" : "text-accent-red"}`}
                  >
    ...
    <div className="bg-surface-card rounded-xl p-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <p className="text-lg font-bold text-accent-green font-mono">
            {data.bullish_pct != null
              ? `${(data.bullish_pct * 100).toFixed(0)}%`
              : "N/A"}
          </p>
        </div>
        <div>
          ...
          <p className="text-lg font-bold text-accent-red font-mono">
            {data.bearish_pct != null
              ? `${(data.bearish_pct * 100).toFixed(0)}%`
              : "N/A"}
          </p>
        </div>
        <div>
          ...
          <p className="text-lg font-bold text-white font-mono">
            {data.company_news_score != null
              ? data.company_news_score.toFixed(2)
              : "N/A"}
          </p>
        </div>
      </div>
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-white font-mono">
            {data.articles_in_last_week ?? "N/A"}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-500">Sector Avg Bullish</span>
          <span className="text-white font-mono">
            {data.sector_avg_bullish != null
              ? `${(data.sector_avg_bullish * 100).toFixed(0)}%`
              : "N/A"}
          </span>
        </div>
        <div className="flex justify-between">
          ...
          <tbody>
            {data.slice(0, 10).map((t, i) => (
              <tr key={i} className="border-b border-border/50">
                ...
                <td
                  className={`py-1 ${t.transaction_type.toLowerCase().includes("sale") ? "text-accent-red" : "text-accent-green"}`}
                >
                  {t.transaction_type}
                </td>
                <td className="text-right text-white font-mono">
          ...
          <tbody>
            {data.slice(0, 10).map((h, i) => (
              <tr key={i} className="border-b border-border/50">
                ...
                <td className="text-right text-white font-mono">
                  {h.value ? `$${(h.value / 1e6).toFixed(0)}M` : "N/A"}
                </td>
                <td className="text-right text-white font-mono">
                  {h.pct_held != null
                    ? `${(h.pct_held * 100).toFixed(2)}%`
                    : "N/A"}
                </td>
              </tr>
            ))}
...
function OptionsCard({ data }: { data: OptionsChain }) {
  ...
  return (
    <div className="bg-surface-card rounded-xl p-4">
      <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">
        Options (nearest expiration: {data.expirations[0]})
      </h3>
          ...
          {data.calls
            ...
            .map((c: Record<string, unknown>, i: number) => (
              <div key={i} className="flex justify-between text-xs py-0.5">
                ...
                <span className="text-white font-mono">
                  ${(c.lastPrice as number)?.toFixed(2) ?? "N/A"}
                </span>
                <span className="text-gray-500">
                  Vol: {(c.volume as number) ?? 0}
                </span>
              </div>
            ))}
        </div>
          ...
          {data.puts
            ...
            .map((p: Record<string, unknown>, i: number) => (
              <div key={i} className="flex justify-between text-xs py-0.5">
                ...
                <span className="text-white font-mono">
                  ${(p.lastPrice as number)?.toFixed(2) ?? "N/A"}
                </span>
                <span className="text-gray-500">
                  Vol: {(p.volume as number) ?? 0}
                </span>
              </div>
            ))}
        </div>
...
function MacroTab() {
  ...
  const factorAverages = useMemo(() => {
    ...
    return keys.map((k) => ({
      factor: k,
      mean:
        recent.reduce((acc, f) => acc + (Number(f[k]) || 0), 0) / recent.length,
    }));
  }, [famaFrench.data]);
  ...
  return (
    <>
      ...
      <div className="bg-surface-card rounded-xl p-4">
        ...
        {macro.isLoading && (
          <p className="text-sm text-gray-500">Loading {seriesLabel}…</p>
        )}
        {macro.isError && (
          <p className="text-sm text-amber-300">
            Macro unavailable: {(macro.error as Error).message}
          </p>
        )}
        ...
        <p className="text-label text-gray-600 mt-2">
          Data: FRED via pandas-datareader · 5y window · change series from the
          dropdown to explore rates, inflation, growth, and liquidity.
        </p>
      </div>
      ...
      {factorAverages.length > 0 && (
        <div className="bg-surface-card rounded-xl p-4">
          <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">
            Fama-French 5-factor · trailing 24m mean (
            {famaFrench.data?.period ?? "monthly"})
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
            ...
            <div className="text-xs text-gray-400 space-y-2">
              <p>
                <span className="text-gray-500 uppercase tracking-wider text-label">
                  Mkt-RF
                </span>{" "}
                — Excess market return. Positive = risk-on backdrop.
              </p>
              <p>
                <span className="text-gray-500 uppercase tracking-wider text-label">
                  SMB
                </span>{" "}
                — Small-minus-big. Positive = small caps outperforming.
              </p>
              <p>
                <span className="text-gray-500 uppercase tracking-wider text-label">
                  HML
                </span>{" "}
                — High-minus-low book/mkt. Positive = value regime.
              </p>
              <p>
                <span className="text-gray-500 uppercase tracking-wider text-label">
                  RMW
                </span>{" "}
                — Robust-minus-weak profits. Positive = quality bid.
              </p>
              <p>
                <span className="text-gray-500 uppercase tracking-wider text-label">
                  CMA
                </span>{" "}
                — Conservative-minus-aggressive investment. Positive =
                capital-discipline bid.
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {famaFrench.data.factors.slice(-12).map((f, i) => (
                  <tr key={i} className="border-b border-border/50">
                    ...
                    <td
                      className={`text-right font-mono ${f["Mkt-RF"] >= 0 ? "text-accent-green" : "text-accent-red"}`}
                    >
                      {f["Mkt-RF"]}
                    </td>
                    <td
                      className={`text-right font-mono ${f.SMB >= 0 ? "text-accent-green" : "text-accent-red"}`}
                    >
                      {f.SMB}
                    </td>
                    <td
                      className={`text-right font-mono ${f.HML >= 0 ? "text-accent-green" : "text-accent-red"}`}
                    >
                      {f.HML}
                    </td>
                    <td
                      className={`text-right font-mono ${f.RMW >= 0 ? "text-accent-green" : "text-accent-red"}`}
                    >
                      {f.RMW}
                    </td>
                    <td
                      className={`text-right font-mono ${f.CMA >= 0 ? "text-accent-green" : "text-accent-red"}`}
                    >
                      {f.CMA}
                    </td>
  ...
  if (sentiment?.bullish_pct != null && sentiment.sector_avg_bullish != null) {
    ...
    if (Math.abs(gap) > 0.2) {
      out.push({
        kind: "edge",
        title:
          gap > 0 ? "Outpacing sector sentiment" : "Lagging sector sentiment",
...
function committeeSpread(
  ...
  if (
    ...
    return {
      ...
      spread:
        dissent.conviction_spread ??
        dissent.conviction_max - dissent.conviction_min,
      mean:
        dissent.conviction_mean ??
        (dissent.conviction_min + dissent.conviction_max) / 2,
    ...
    <div className="rounded-xl border border-blue-500/25 bg-surface-card p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-label text-gray-600">
          where data disagrees, where edge lives
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="md:col-span-2 flex flex-col gap-2">
          {callouts.length === 0 ? (
            <div className="text-xs text-gray-500 italic p-3 rounded border border-dashed border-border/60">
              No non-consensus signals yet — fetch analyst / sentiment tabs or
              rerun AI to populate.
            </div>
          ) : (
            callouts.map((c, i) => (
              <div
        ...
        <div className="rounded-lg border border-border/60 bg-black/30 p-3 text-xs flex flex-col gap-2">
          <p className="text-label uppercase tracking-wider text-gray-500">
            Committee spread
          </p>
          {spread ? (
            <>
                ...
                <div className="mt-2 pt-2 border-t border-border/40 space-y-1">
                  ...
                  {refinement.suspect_convergence && (
                    <p className="text-label text-red-300 leading-relaxed">
                      Every dissenting analyst moved to the same view in one
                      round — treat as deference to the group, not independent
                      agreement.
                    </p>
                  )}
                </div>
            ...
            <p className="text-gray-500 italic">
              Run committee mode to see spread.
            </p>
          )}
        </div>
      </div>
...
function InsiderFlowCard({
  ...
  return (
    <div className="bg-surface-card rounded-xl p-4">
      <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">
        Insider flow (last 50)
      </h3>
      {loading && <p className="text-sm text-gray-500">Loading…</p>}
      {!loading && data.length === 0 && (
        <p className="text-sm text-gray-500">No recent insider activity.</p>
      )}
      {!loading && data.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 gap-2 mt-3 text-xs">
            <div>
              ...
              <p className="text-gray-500 font-mono">
                ${(net.buy / 1e6).toFixed(1)}M
              </p>
            </div>
            <div className="text-right">
              <p className="text-red-400">Sells · {net.sellCount}</p>
              <p className="text-gray-500 font-mono">
                ${(net.sell / 1e6).toFixed(1)}M
              </p>
            </div>
          </div>
          <div className="mt-3 pt-3 border-t border-border/50">
            <p className="text-label text-gray-600 uppercase tracking-wider mb-1">
              Recent
            </p>
          ...
          {data.slice(0, 8).map((t, i) => (
            <div key={i} className="flex justify-between items-start text-xs">
              ...
              <div className="text-right shrink-0 ml-2">
                ...
                <p className="text-label text-gray-500 truncate max-w-[8rem]">
                  {t.amount_range ?? "—"}
                </p>
              </div>
            </div>
...
function ESGCard({
  ...
  return (
    <div className="bg-surface-card rounded-xl p-4">
      ...
      {!loading && !data && (
        <p className="text-sm text-gray-500">
          ESG unavailable (set FINNHUB_API_KEY).
        </p>
      )}
      {!loading && data && (
        <>
          ...
          <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
            ...
            <div>
              ...
              <p className="text-gray-200 font-mono">
                {data.governance_score != null
                  ? data.governance_score.toFixed(0)
                  : "—"}
              </p>
            </div>
          </div>
...
function FilingsCard({
  ...
  if (!data?.length) {
    return (
      <div className="bg-surface-card rounded-xl p-4">
        ...
        <p className="text-sm text-gray-500">
          No filings available (requires Finnhub or EDGAR access).
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-surface-card rounded-xl p-4">
          ...
          <tbody>
            {data.slice(0, 15).map((f, i) => (
              <tr key={i} className="border-b border-border/50">
                ...
                <td className="text-gray-300 py-1 truncate max-w-xs">
                  {f.description ?? "—"}
                </td>
      ...
      <p className="text-label text-gray-600 mt-2">
        Form 4 = insider · 13F = institutional · 8-K = material events · 10-K/Q
        = financials.
      </p>
    </div>
  );
    ...
    <div className="flex flex-col gap-4">
                ...
                return (
                  ...
                  >
                    <td className="py-2 pr-3">
                      ...
                      {isSelf && (
                        <span className="text-label text-blue-400/80 ml-2">
                          (this)
                        </span>
                      )}
                    </td>
                    {PEER_METRIC_KEYS.map((m) => {
                      ...
                      return (
                        <td
                          key={m.key}
                          className="text-right py-2 px-2 font-mono text-gray-300"
                        >
                          {typeof v === "number"
                            ? m.fmt
                              ? m.fmt(v)
      ...
      {rows.length > 1 && radarData.length > 0 && (
        <div className="bg-surface-card rounded-xl p-4">
          ...
          <p className="text-label text-gray-600 mt-2">
            Each axis is normalized against the peer maximum — larger polygon =
            stronger relative profile. Compare shape differences to spot where
            this name leads or lags peers.
          </p>
        </div>
      )}
      ...
      <div className="bg-surface-card rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(
            ...
            const label =
              PEER_METRIC_KEYS.find((m) => m.key === key)?.label ?? key;
            ...
            return (
              <div key={key} className="h-40">
                <p className="text-label text-gray-500 uppercase tracking-wider mb-1">
                  {label}
                </p>
      ...
      <div className="bg-surface-card rounded-xl p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <p className="text-label text-gray-500 ml-auto max-w-sm">
            No look-ahead · 5 bps fee per side · signals computed on close,
            executed next bar.
          </p>
        </div>
      ...
      {d && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <SmallStat
              label="Round-trips"
              value={String(d.metrics.num_trades)}
            />
                  ...
                  <tbody>
                    {d.trades.map((t, i) => (
                      <tr key={i} className="border-b border-border/50">
                        ...
                        <td className="text-right text-gray-500 font-mono">
                          {t.days_held}
                        </td>
                        <td
