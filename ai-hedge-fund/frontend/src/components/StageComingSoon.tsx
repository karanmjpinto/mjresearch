...
import { YourLens } from "@/components/YourLens";
...
const PLANNED: Record<string, { phase: string; bullets: string[] }> = {
  lens: {
    phase: "Phase 3",
    bullets: [
      "Your frameworks applied rather than listed — each lens run against this company's numbers, not just pointed at.",
      "No page on this name yet? Draft a Value One Pager in your own template, pre-linked to the notes it matched.",
      "Write back: the decision, the price and the reasoning returned to the vault as a note you can find later.",
    ],
  },
  value: {
    phase: "Phase 2",
...
export function StageComingSoon({
  ...
  stage: Extract<StageKey, "lens" | "value">;
}) {
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-ink">
      ...
      <main className="mx-auto w-full max-w-4xl px-lg py-2xl">
        ...
        <p className="mt-sm max-w-[60ch] text-body-lg text-on-ink-soft">
          {spec?.question}
        </p>
        ...
        {stage === "lens" && ticker && (
          <div className="mt-xl border-t border-ink-line pt-lg">
            <YourLens ticker={ticker.toUpperCase()} />
          </div>
        )}
        ...
        <div className="mt-xl border-l-2 border-cadmium pl-md">
          ...
          <ul className="mt-sm flex flex-col gap-sm">
            {planned.bullets.map((b) => (
              <li
                key={b}
                className="max-w-[68ch] text-body-sm text-on-ink-soft"
              >
                {b}
              </li>
            ))}
        ...
        <p className="mt-xl max-w-[68ch] text-body-sm text-on-ink-faint">
          The stages either side of this one are live — use the rail above to
          move between them.
        </p>
      </main>
    </div>
