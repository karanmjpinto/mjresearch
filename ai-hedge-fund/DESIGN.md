...
| 2026-09-13 | Raised both `faint` text steps to clear 4.5:1 | Measured, not guessed. `on-ink-faint` was 3.65:1 on ink and 3.25:1 on a raised panel; `on-canvas-faint` was worse at 2.93:1. Both carry 12px uppercase labels — the smallest type in the system and the least forgiving. `on-ink-faint` went L 0.54 → 0.62 (5.07:1 / 4.52:1) and `on-canvas-faint` L 0.62 → 0.515 (4.52:1). Both keep a clear step from their `soft` neighbour, so three levels still read as three. |
| 2026-09-13 | `title-xs` 1.0625rem → 1.1875rem, `title-sm` → 1.375rem | `title-xs` and `body-lg` were the *identical* size, so that step of the scale did not exist: a card heading was distinguished from body copy by weight and colour alone. Flagged by a design review and confirmed by reading the config. |
| 2026-09-13 | One declared light, `--light-*` + `shadow-elev-1/2` | Borrowed from [Ambient CSS](https://ambientcss.vercel.app/): a page declares a single light direction and each raised surface derives its shading from it, instead of every component inventing a `box-shadow`. Two things follow that ad-hoc shadows cannot give — every panel on a screen is lit from the same place, and moving the light is one line. Implemented in our own tokens rather than adding the library: its glass, brushed and spun materials belong to a different product, and a React dependency for visual polish is a poor trade. Elevation stops at 2 on purpose; a research tool needing a third stacking level has a layout problem, not a lighting one. Applied only to surfaces already claiming to be raised (`bg-ink-raised`), so the light says *which things sit above the ground* and nothing else. Finish stays matte — no specular highlight in a system derived from paint on canvas. |
